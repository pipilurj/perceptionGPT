...
class SingleImageConvDatasetMixin:
    ...
    def __getitem__(self, index, debug_mode=False) -> Dict[str, Any]:
        ...
        # preprocess
        image_ori = image
        raw_conv = self.process_conv(raw_conv) # replace all the <image> tokens
        raw_conv, image = self.process_conv_multimage(raw_conv, image) # process multi-images, in our case, does nothing
        # TODO: modify here to support indication of input/output box and mask
        raw_conv, _, boxes_seq, points_seq, masks_seq = self.process_target(raw_conv, target, multimage_mode=multimage_mode) # box, mask
        conv = self.build_conv(raw_conv)
        text_dict = self.process_text(conv) # encode the text
        image_dict = self.process_image(image) # proceess image, do the resize and stuff
        ...
        ret_dict.update({"image_ori": image_ori})
        if target is not None:
            ret_dict.update({"expr": target.get('expr', "None")})
        return ret_dict
