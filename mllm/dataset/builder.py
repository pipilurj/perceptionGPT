...
def prepare_data(
    ...
    if hasattr(data_args, 'multival') and bool(data_args.multival):
        print(f"processing multival set")
        k2v = {}
        for k, item in data_args.multival.items():
            _dataset_cls = partial(DATASETS.build, item['cfg'])
            k2v[k] = conv_dataset_cls(dataset_generator=_dataset_cls, mode='train')
        ds['multival'] = k2v
        print(f"processing multival set. done.")
