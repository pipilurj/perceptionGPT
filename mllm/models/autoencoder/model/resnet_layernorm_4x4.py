...
class ResNet(nn.Module):
    def __init__(self, downblock, upblock, num_layers, n_classes, norm_enc="ln", norm_dec="ln"):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1) #  64x64

        self.dlayer1 = self._make_downlayer(downblock, 128, num_layers[0], norm=norm_enc) #64x64
        self.dlayer2 = self._make_downlayer(downblock, 256, num_layers[1],
                                            stride=2, norm=norm_enc)  # 32 x 32
        self.dlayer3 = self._make_downlayer(downblock, 512, num_layers[2],
                                            stride=2, norm=norm_enc) # 16x16
        self.dlayer4 = self._make_downlayer(downblock, 1024, num_layers[3],
                                            stride=2, norm=norm_enc) # 4 x 4
        ...
        #                        bias=False)
        # self.linear_enc = nn.Linear(4096*4*4, 4096)
        # self.linear_dec = nn.Linear(4096, 4096*4*4)
        self.uplayer1 = self._make_up_block(upblock, 1024, 1, stride=2, norm=norm_dec) # 16x16
        self.uplayer2 = self._make_up_block(upblock, 512, num_layers[2], stride=2, norm=norm_dec) # 32x32
        self.uplayer3 = self._make_up_block(upblock, 128, num_layers[1], stride=2, norm=norm_dec) # 64x64
        self.uplayer4 = self._make_up_block(upblock, 64, num_layers[0], stride=2, norm=norm_dec) # 128x128
        ...
        upsample = nn.Sequential(
            ...
            norm_map[norm_dec](32),
        )
        self.uplayer_top = DeconvBottleneck(self.in_channels, 32, 1, 2, upsample)

        self.conv1_1 = nn.ConvTranspose2d(32, n_classes, kernel_size=1, stride=1,
                                          bias=False)
