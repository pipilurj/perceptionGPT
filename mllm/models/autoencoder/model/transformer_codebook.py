...
class TransformerLocCodebook(nn.Module):
    ...
    def forward(self, locations, mask_enc=None, mask_dec=None, loc_flags=None, return_embedding=False):
        ...
        if self.attn_with_pos:
            pos_enc = self.pos_embed_enc(mask_enc)
        else:
            pos_enc = None
        ...
        if self.attn_with_pos:
            ...
            pos_dec = None
        output = self.decoder(locations_embed_enc, hidden_repr_dec, trg_mask=trg_mask, src_mask=src_mask, pos=pos_dec)
        return {
            "output":output,
...
if __name__ == "__main__":
    ...
    val_loader = DataLoader(val_dataset, batch_size=100, collate_fn=custom_collate_fn, num_workers=0)
    gts, preds = [], []
    with torch.no_grad():
