...
class ResNet(nn.Module):
    def __init__(self, downblock, upblock, num_layers, n_classes, norm_enc="ln", norm_dec="ln"):
        ...
        self.uplayer1 = self._make_up_block(upblock, 128, 1, stride=2, norm=norm_dec) # 16x16
        self.uplayer2 = self._make_up_block(upblock, 64, 2, stride=2, norm=norm_dec) # 32x32
        self.uplayer3 = self._make_up_block(upblock, 32, num_layers[2], stride=2, norm=norm_dec) # 64x64
        self.uplayer4 = self._make_up_block(upblock, 16, num_layers[1], stride=2, norm=norm_dec) # 128x128
        ...
        self.uplayer_top = DeconvBottleneck(self.in_channels, 16, 1, 2, upsample) # 256x256
...
class ResNetEncoder(nn.Module):
    def __init__(self, downblock, num_layers, norm_enc="ln"):
        ...
        self.dlayer4 = self._make_downlayer(downblock, 32, num_layers[3],
                                            stride=2, norm=norm_enc)
        self.linear_enc = nn.Linear(32*2*8*8, 512)
        self.linear_dec = nn.Linear(512, 4096)
