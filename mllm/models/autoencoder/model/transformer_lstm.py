...
from models.model.decoder import MLP
...
from models.embedding.transformer_embedding import TransformerEmbedding, TransformerEmbeddingWithLoc
...
class DecoderLoc(nn.Module):
    ...
    def forward(self, locations, enc_src, trg_mask=None, src_mask=None, pos=None):
        ...
        trg = self.emb(locations, ifdecode=True, add_pos=True)
...
class TransformerLSTM(nn.Module):
    ...
    def forward(self, locations, mask_enc=None, mask_dec=None, loc_flags=None, return_embedding=False):
        ...
        coords = locations["polygon"].to(device)[:,:-1]#.reshape(len(quant_poly11), -1)
        locations_embed_enc = self.coord2embedenc(locations["polygon"].to(device))
