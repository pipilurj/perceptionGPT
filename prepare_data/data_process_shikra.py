...
def prepare_dataset(dataset, splits, output_dir, generate_mask=False):
    ann_path = os.path.join(output_dir, 'anns', dataset, args.split)
    mask_path = os.path.join(output_dir, 'masks', dataset, args.split)
    ...
    for split in splits:
        ...
        for i in tqdm(ref_ids):
            ...
            if generate_mask:
                ...
                ann = refer.refToAnn[refs['ref_id']]
                ref_dict['segmentation'] = ann["segmentation"]
        ...
        with open(os.path.join(output_dir, 'anns', dataset, args.split, split + "_mask"+ '.jsonl'),
                  'w') as outfile:
